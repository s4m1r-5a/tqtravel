...
if (window.location == `${window.location.origin}/links/calendario`) {
    ...
    function initMap() {
        var map = new google.maps.Map(document.getElementById('map'), {
            ...
            center: { lat: 11.2407904, lng: -74.1990433 },
            zoom: 13
        });
    ...
    function AutocompleteDirectionsHandler(map) {
        ...
        originAutocomplete.setComponentRestrictions(
            { 'country': ['co'] });
        originAutocomplete.setFields(['address_components', 'geometry', 'icon', 'place_id', 'name']);
        ...
        originAutocomplete.setComponentRestrictions(
            { 'country': ['co'] });
        destinationAutocomplete.setFields(['address_components', 'geometry', 'icon', 'place_id', 'name']);
    ...
    AutocompleteDirectionsHandler.prototype.setupPlaceChangedListener = function (
        ...
        autocomplete.addListener('place_changed', function () {
            ...
            filt = place.address_components
                .filter(function (ruta) {
                    return ruta.types[0] === "administrative_area_level_2" || ruta.types[0] === "administrative_area_level_1"
                })
                .map(function (ruta) {
                    return ruta.short_name
                })
            console.log(place.address_components)
            console.log(`${place.name}, ${filt[0]}`)
            if (!place.place_id) {
    ...
    //Cambiar formato de AM/PM a 24 HORAS
    function HHmm24(time) {
        if (time.indexOf('M') > 0) {
            var [fecha, hora, M] = time.split(' ');
            var hours = parseInt(hora.slice(0, 2));
            if (M == 'AM' && hours == 12) {
                hora = hora.replace('12', '0');
            }
            if (M == 'PM' && hours < 12) {
                hora = hora.replace(hora.slice(0, 2), (hours + 12));
            }
    ...
    function RecolectarDatosGUI() {
        NuevoEvento = {
            ...
            usuario: $('#user').val(),
            creador: $('.user').html(),
            valor: $('#precio').val().replace(/(?!\w|\s).| /g, "") || 0,
            vuelo: $('#vuelo').val(),
    ...
    $('#fullcalendar').fullCalendar({
        ...
        eventAfterRender: function (event, element, view) {
            if (event.factura > 0) {
                ...
                element.css('background-color', '#3374FF');
            }
        },
        ...
        eventClick: function (calEvent, jsEvent, view) {
            ...
            $('#origen').val(calEvent.partida);
            $('#destino').val(calEvent.destino);
            $('#descripcion').val(calEvent.observaciones);
            $('#observacion').val(calEvent.adicionales);
            $('#grupo').val(calEvent.grupo);
